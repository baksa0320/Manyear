...
$(function(){
    ...
    $("#commentParentSubmit").click(function( event ) {
        ...
        if($.trim($("#commentParentName").val())==""){
            alert("이름을 입력하세요.");
            pName.focus();
            return;
        }else if($.trim($("#commentParentPassword").val())==""){
            alert("패스워드를 입력하세요.");
            pPassword.focus();
            return;
        }else if($.trim($("#commentParentText").val())==""){
            alert("내용을 입력하세요.");
            pText.focus();
            return;
        }
    	 ...
    	 $.ajax({
 	        ...
 	        data: {
 	        	num : $('#num').val(),
        		name : $('#commentParentName').val(),
        		pwd : $('#commentParentPassword').val(),
    ...
    $("#commentParentSubmit2").click(function( event ) {
    ...
    function whenSuccess(resdata){
        ...
        var commentParentText = '<tr id="r1" name="commentParentCode">'+
                                    '<td width="20%"><strong>' + pName.val() + '</strong></td>' +
                                    '<td>' + pText.val() + '</td>' + 
                                '</tr>';
                                
        //테이블의 tr자식이 있으면 tr 뒤에 붙인다. 없으면 테이블 안에 tr을 붙인다.
        if($('#commentTable').contents().size() == 1){
